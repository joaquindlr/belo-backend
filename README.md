...
5.  Muy importante: decidí implementar /transactions enviando el id del usuario como Query Param en lugar de como Path Param, por dos motivos:
    1.  Por convencion de Rest, normalmente un Path Param identifica el recurso, por ejemplo en transactions deberia identificar e individualizar una transaccion, no las transacciones de un usuario. En cambio, Query Params es la convencion para filtrar recursos. En un caso en producción probablemente un usuario solo pueda consultar las transacciones de su propio usuario, y ahi extraeriamos el userId del token, pero me imagino un caso en el que un operador tiene permisos para revisar todas las transacciones, y en ese caso utilizaria un filtro por Query Params.
    2.  Hacerlo de esta forma me permite reutilizar el endpoint en la pantalla de dashboard para listar todas las transacciones (ademas de poder agregar un filtro "ALL" en vez de listar la de un usuario a la vez) y en la pantalla de aprobaciones.
